/**
 * Joined Contexts Page
 * Display contexts (events, groups, conventions) the user has joined
 */
...
import { ArrowLeft, Users, Calendar, Loader2, Search } from 'lucide-react';
...
export default function JoinedPage({ params }: JoinedPageProps) {
  ...
  const {
    ...
    hasNextPage,
    isFetchingNextPage,
  } = trpc.user.getJoinedContexts.useInfiniteQuery(
    {
      userId: user?.id ?? '',
    ...
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      ...
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-display font-bold text-forest-800 dark:text-cream-100 font-korean">
          {user.displayName || user.username}님이 참여 중
        </h1>
        <p className="text-warm-500 dark:text-warm-400 font-korean mt-1">
          {contexts.length}개의 그룹/이벤트에 참여하고 있습니다
        </p>
      </div>
      ...
      {!isLoading && contexts.length === 0 && (
        <div className="text-center py-16">
          <Search className="w-16 h-16 mx-auto mb-4 text-warm-300 dark:text-warm-600" />
          ...
          <div className="flex gap-3 justify-center">
            ...
            >
              ...
              <span className="font-korean">그룹 찾기</span>
            </Link>
            <Link
              href="/events"
            ...
            >
              ...
              <span className="font-korean">이벤트 찾기</span>
            </Link>
          </div>
        </div>
      )}
